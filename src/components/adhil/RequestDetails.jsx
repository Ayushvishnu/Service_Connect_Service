...
export default function RequestDetails({ showButtons }) {
      const [activeTab, setActiveTab] = useState("request");
      const [showLeadModal, setShowLeadModal] = useState(false);

        const leadBalance = 50 
  const hasLeadBalance = leadBalance > 0;
  ...
  <div className="w-full flex flex-col mt-5">
    ...
    {activeTab === "request" && (
      <>
        <Link
          to="/75"
          className="w-full md:w-[85%] h-10 md:h-12 self-center group relative flex items-center justify-center rounded-full bg-gray-800 px-6 py-3 font-medium text-white hover:bg-gray-900"
        >
          <span>Accept & Schedule</span>
          <div className="absolute right-2 h-8 w-8 flex items-center justify-center rounded-full bg-white group-hover:scale-110 transition">
    ...
    {activeTab === "address" && (
      <div className="flex gap-4 w-full md:w-[85%] self-center mb-10">
